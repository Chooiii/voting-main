...
def update_value():
    ...
    if result:
        ...
        if input_name:
            # Check if a vote for president already exists
        ...
        if vice_president:
            # Check if a vote for vice president already exists
        ...
        if secretary:
            # Check if a vote for secretary already exists
        ...
        if treasurer:
            # Check if a vote for treasurer already exists
...
def update_values():
    ...
    if result:
        flash("THIS ID IS ALREADY TAKEN USE ANOTHER ID")
...
def newcandi():
    ...
    if request.method == 'POST':
        ...
        if table_name:
            ...
            if existing_table:
                ...
                # Dynamically create a table with the specified name
                cur = mysql.connection.cursor()
                ...
                cur.execute(f"INSERT INTO {table_name} (boto,numbercheck) VALUES (%s,%s)", (0, numbercheck))
                mysql.connection.commit()
                cur.close()
    ...
    return render_template('newPosition.html', tables_and_rows=tables_and_rows, table=table_name)
...
def candidatesko():
    tables_and_rows = fetch_tables_and_rows()
    return render_template('candidates.html', tables_and_rows=tables_and_rows, )
...
def deletessssss(table, id_data):
    ...
    return redirect(url_for('candidatesko'))  # Replace 'some_redirect_function' with the actual function to redirect to
...
def update_record(table, id_data):
    ...
    if request.method == 'POST':
        ...
        # Check if the new name already exists in the database excluding the current record
        cur.execute("SELECT * FROM {} WHERE name=%s AND id!=%s".format(table), (new_name, id_data))
        existing_record = cur.fetchone()

        if existing_record:
            # If the name already exists (excluding the current record), flash a message and redirect
            flash("Record with name '{}' already exists".format(new_name), "danger")
            cur.close()
            return redirect(url_for('candidatesko'))
        else:
            # If the name doesn't exist, perform the update
    ...
    # If it's a GET request, you might want to fetch the current data and pass it to the template
...
def newinsert(table):
    ...
    if request.method == 'POST':
        ...
        if existing_record:
            # If the name already exists, flash a message and redirect
            flash("Record with name '{}' already exists".format(name), "danger")
            cur.close()
            return redirect(url_for('candidatesko'))
        else:
            # If the name doesn't exist, insert the new record
            cur.execute("INSERT INTO {} (name, boto) VALUES (%s, 0)".format(table), (name,))
            mysql.connection.commit()
            cur.close()
            flash("Record Has Been Updated Successfully", "success")
            return redirect(url_for('candidatesko'))

    return render_template('newPosition.html', table=table)
def mgatoto():
    cur = mysql.connection.cursor()
    cur.execute("SHOW TABLES")
    tables = [row[0] for row in cur.fetchall()]
    ...
    for table in tables:
        if table != 'botomo' and table != 'user':
...
def voters():
    ...
    return render_template('voters.html', tables_and_rows=tables_and_rows,  )
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            return redirect(url_for('newcandi'))
        else:
            msg = 'Incorrect username/password!!'

    return render_template('lagin.html', msg=msg)
